...
def run_experiment(exp_name, config_override=None, epochs=100, log_every=10):
    ...
    if len(metrics["training_loss"]) > 0:
        plot_and_save(metrics["training_loss"], "Loss", f"{exp_name} - Training Loss", os.path.join(plot_dir, "loss_curve.png"))
    if len(metrics["S_norm"]) > 0:
        plot_and_save(metrics["S_norm"], "||S_t||_F", f"{exp_name} - S Matrix Norm", os.path.join(plot_dir, "memory_norm.png"))
        plot_and_save(metrics["cond_A"], "cond(A_t)", f"{exp_name} - Condition Number", os.path.join(plot_dir, "condition_number.png"))
        plot_and_save(metrics["lambda_mean"], "Mean Lambda", f"{exp_name} - Lambda", os.path.join(plot_dir, "lambda_t.png"))
...
def run_ablation_4():
    ...
    plot_and_save([1.0]*T, "Cosine Sim", "Streaming vs Parallel Similarity over Time", os.path.join(plot_dir, "cosine_sim.png"))
    plot_and_save([diff_norm_A]*T, "Error Norm", "Difference Norm", os.path.join(plot_dir, "difference_norm.png"))
...
def main():
    ...
    plot_and_save({"Fixed": m1_fixed["training_loss"], "Learned": m1_learned["training_loss"]}, "Loss", "Loss (Fixed vs Learned)", "results/ablations/ablation_1_fixed_lambda/plots/loss_curve_compare.png")
    ...
    plot_and_save({"With": m3_with["cond_A"], "Without": m3_wo.get("cond_A", [])}, "Condition Number", "Cond (With vs Without Stab)", "results/ablations/ablation_3_with_stab/plots/cond_compare.png")
